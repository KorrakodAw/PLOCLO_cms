...
/////////////////////////////////////////////////////////////////////////
router.get(
  "/ass-clo/allStudentCourse/percentage",
  authenticateToken,
  async (req, res) => {
    const { CsemesterId } = req.query;
    try {
      const resultCloPerStudent = await prisma.$transaction(async (tx) => {
        return await getCloPercentageAllStudentPerCourse(
          tx,
          Number(CsemesterId),
        );
      });
...
/////////////////////////////////////////////////////////////////////////
router.get(
  "/ass-clo/course/stats/percentage",
  authenticateToken,
  async (req, res) => {
    const { CsemesterId } = req.query;
    try {
...
/////////////////////////////////////////////////////////////////////////
router.get(
  "/realScoreAndGrade/allStudentCourse/percentage",
  authenticateToken,
  async (req, res) => {
    const { CsemesterId } = req.query;
    try {
      const resultCloPerStudent = await prisma.$transaction(async (tx) => {
        return await getRealScorePercentageAllStudentPerCourse(
          tx,
          Number(CsemesterId),
        );
      });
...
/////////////////////////////////////////////////////////////////////////
router.get(
  "/realScoreAndGrade/stats/percentage",
  authenticateToken,
  async (req, res) => {
    const { CsemesterId } = req.query;
    try {
      const resultCloPerStudent = await prisma.$transaction(async (tx) => {
        return await getRealScoreStatsPercentagePerCourse(
          tx,
          Number(CsemesterId),
        );
      });
...
router.get("/clo-plo/allStudentCourse", authenticateToken, async (req, res) => {
  ...
  try {
    const resultPloStudent = await prisma.$transaction(async (tx) => {
      return await getPloScoreAllStudentPerCourse(
        tx,
        Number(CsemesterId),
        Number(courseId),
      );
...
/////////////////////////////////////////////////////////////////////////
router.get(
  "/clo-plo/allStudentCourse/percentage",
  authenticateToken,
  async (req, res) => {
    const { CsemesterId, courseId } = req.query;
    try {
      const resultPloStudent = await prisma.$transaction(async (tx) => {
        return await getPloPercentageAllStudentPerCourse(
          tx,
          Number(CsemesterId),
          Number(courseId),
        );
...
/////////////////////////////////////////////////////////////////////////
router.get(
  "/clo-plo/allStudentSemester",
  authenticateToken,
  async (req, res) => {
    const { programId, semester, year } = req.query;
    try {
      const resultPloStudent = await prisma.$transaction(async (tx) => {
        return await getPloScoreAllStudentPerSemester(
          tx,
          Number(programId),
          Number(year),
          Number(semester),
...
/////////////////////////////////////////////////////////////////////////
router.get(
  "/clo-plo/allStudentSemester/percentage",
  authenticateToken,
  async (req, res) => {
    const { programId, semester, year } = req.query;
    try {
      const resultPloStudent = await prisma.$transaction(async (tx) => {
        return await getPloScoreAllStudentPerSemesterPercentage(
          tx,
          Number(programId),
          Number(year),
          Number(semester),
...
router.get("/clo-plo/allStudentYear", authenticateToken, async (req, res) => {
  ...
  try {
    const resultPloStudent = await prisma.$transaction(async (tx) => {
      return await getPloScoreAllStudentPerYear(
        tx,
        Number(programId),
        Number(year),
      );
...
/////////////////////////////////////////////////////////////////////////
router.get(
  "/clo-plo/allStudentYear/percentage",
  authenticateToken,
  async (req, res) => {
    const { programId, year } = req.query;
    try {
      const resultPloStudent = await prisma.$transaction(async (tx) => {
        return await getPloScoreAllStudentPerYearPercentage(
          tx,
          Number(programId),
          Number(year),
        );
...
router.get("/clo-plo/course/stats", authenticateToken, async (req, res) => {
  ...
  try {
    const resultCloStudent = await prisma.$transaction(async (tx) => {
      return await getPloStatsPerCourse(
        tx,
        Number(CsemesterId),
        Number(courseId),
      );
...
/////////////////////////////////////////////////////////////
router.get(
  "/clo-plo/course/stats/percentage",
  authenticateToken,
  async (req, res) => {
    const { CsemesterId, courseId } = req.query;
    try {
      const resultCloStudent = await prisma.$transaction(async (tx) => {
        return await getPloStatsPercentagePerCourse(
          tx,
          Number(CsemesterId),
          Number(courseId),
        );
...
router.get("/clo-plo/semester/stats", authenticateToken, async (req, res) => {
  ...
  try {
    const resultCloStudent = await prisma.$transaction(async (tx) => {
      return await getPloStatsPerSemester(
        tx,
        Number(programId),
        Number(year),
        Number(semester),
...
/////////////////////////////////////////////////////////////
router.get(
  "/clo-plo/semester/stats/percentage",
  authenticateToken,
  async (req, res) => {
    const { programId, year, semester } = req.query;
    try {
      const resultCloStudent = await prisma.$transaction(async (tx) => {
        return await getPloStatsPerSemesterPercentage(
          tx,
          Number(programId),
          Number(year),
          Number(semester),
...
/////////////////////////////////////////////////////////////
router.get(
  "/clo-plo/year/stats/percentage",
  authenticateToken,
  async (req, res) => {
    const { programId, year, semester } = req.query;
    try {
      const resultCloStudent = await prisma.$transaction(async (tx) => {
        return await getPloStatsPerYearPercentage(
          tx,
          Number(programId),
          Number(year),
        );
...
/////////////////////////////////////////////////////////////
router.get(
  "/clo-plo/studentCumulative/stats",
  authenticateToken,
  async (req, res) => {
    const { programId } = req.query;
    try {
      // เพิ่มอาร์กิวเมนต์ที่ 2 ต่อจาก async (tx) => { ... }
      const resultPlo = await prisma.$transaction(
        async (tx) => {
          return await getProgramPloDetailedStatistics(tx, Number(programId));
        },
        {
          maxWait: 5000, // เวลารอคิวเพื่อเริ่ม transaction (default: 2000)
          timeout: 20000, // เวลาที่อนุญาตให้ทำงานจนเสร็จ (เพิ่มเป็น 30 วินาที)
        },
      );
