...
interface AssignmentPerformanceTableProps {
  ...
  categoryChartData: CategoryChartDataProps[];
  getGradeColor: (grade: string) => string;
}


export const AssignmentPerformanceTable = ({
  summaryData,
  categoryChartData,
  getGradeColor,
  ...
  const cloNames = React.useMemo(
    () => categoryChartData.map((item) => item.name),
    [categoryChartData],
  );

  if (!summaryData || categoryChartData.length === 0) return null;

  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
      <div className="p-6 border-b border-slate-100 bg-slate-50/30">
        <h3 className="font-bold text-slate-800 flex items-center gap-2">
          <FaBullseye className="text-blue-500" /> Assignment Achievement by Grade
        </h3>
      </div>
...
export default AssignmentPerformanceTable;