...
export default function PloStatsDashboard({
  ...
  }, [data.studentStat, data.studentName, program_id]); // 🟢 2. อย่าลืมใส่ selectedProgramId ใน Dependency
    ...
    return (
      <div className="mt-8 bg-white p-12 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-slate-500 max-w-xs mt-2">
          There is no PLO performance data recorded for the{" "}
          <span className="font-semibold text-blue-600">{CsemesterId}</span>{" "}
          Course <span className="font-semibold text-blue-600">{courseId}</span>{" "}
          yet.
        </p>
      </div>
    );
      ...
      {/* <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
           <h2 className="text-2xl font-bold text-slate-800">
            Dashboard for {CsemesterId} Course {courseId}
          </h2> 
